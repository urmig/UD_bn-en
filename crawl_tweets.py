...
'''Based on Hindi-English Tweet Crawler by I.A.Bhat'''
...
if __name__ == '__main__':
    description = 'Download, tokenize and annotate Bengali-English tweets'
    ...
    for tid in tweet_ids:
        ...
        if tok[0] not in tweets:
            ...
            #op.write(str(tid)+'\t'+tweets[tid]+'\n')
            if tok[0] in edits:
                for edit in edits[ tok[0]]:
                	tweets[ tok[0]] = tweets[ tok[0]].replace(edit[0], edit[1])
            tweet = token.tokenize(tweets[ tok[0]])
            
            #Splitting Sentences within a tweet
            size=len(tweet)
            idx_list = [idx + 1 for idx, val in enumerate(tweet) if (val in ['.', '..', '...', '?', '???', '??!', '?!!', '!', '!!', '!!!', '!!!!' ])] 
            if(idx_list):
